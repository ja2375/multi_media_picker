 ...
 */
public class MultiMediaPickerDelegate
        implements PluginRegistry.ActivityResultListener,
        PluginRegistry.RequestPermissionsResultListener {
    private static final int REQUEST_CODE_CHOOSE_IMAGE_FROM_GALLERY = 2342;
    private static final int REQUEST_CODE_CHOOSE_IMAGES_FROM_GALLERY = 23421;
    private static final int REQUEST_CODE_TAKE_IMAGE_WITH_CAMERA = 2343;
    private static final int REQUEST_EXTERNAL_IMAGE_STORAGE_PERMISSION = 2344;
    private static final int REQUEST_CAMERA_IMAGE_PERMISSION = 2345;
    private static final int REQUEST_CODE_CHOOSE_VIDEO_FROM_GALLERY = 2352;
    private static final int REQUEST_CODE_TAKE_VIDEO_WITH_CAMERA = 2353;
    private static final int REQUEST_EXTERNAL_VIDEO_STORAGE_PERMISSION = 2354;
    private static final int REQUEST_CAMERA_VIDEO_PERMISSION = 2355;
    ...
    public MultiMediaPickerDelegate(
    ...
    public void chooseImageFromGallery(MethodCall methodCall, MethodChannel.Result result) {
        ...
        if(methodCall.argument("singleImage"))
    ...
    private void launchPickImageFromGalleryIntent() {
        ...
        activity.startActivityForResult(Intent.createChooser(pickImageIntent, "Select Picture"), REQUEST_CODE_CHOOSE_IMAGE_FROM_GALLERY);
    }
    ...
    private void launchPickImagesFromGalleryIntent() {
        ...
        pickImageIntent.setAction(Intent.ACTION_GET_CONTENT);
        activity.startActivityForResult(Intent.createChooser(pickImageIntent, "Select Pictures"), REQUEST_CODE_CHOOSE_IMAGES_FROM_GALLERY);
    }
        ...
        if (resultCode == Activity.RESULT_OK && data != null) {
            ...
            if (data.getData() != null) {
                ...
                for (int i = 0; i < clipData.getItemCount(); i++) {
                    String path = fileUtils.getPathFromUri(activity, clipData.getItemAt(i).getUri());
                    imagesList.add(path);
                }
            }
            handleImagesResult(imagesList);
            return;
        }
    ...
    private void handleImageResult(String path) {
        if (pendingResult != null) {
            ...
            imgs.add(finalImagePath);
            finishImagesWithSuccess(imgs);
        } else {
            throw new IllegalStateException("Received image from picker that was not requested");
        }
    }

    private void handleImagesResult(ArrayList<String> paths) {
        if (pendingResult != null) {
            Double maxWidth = methodCall.argument("maxWidth");
            Double maxHeight = methodCall.argument("maxHeight");
            ArrayList<String> finalImagesPath = new ArrayList<>();
            for (int i = 0; i < paths.size(); i++) {
                String finalImagePath = imageResizer.resizeImageIfNeeded(paths.get(i), maxWidth, maxHeight);
                finalImagesPath.add(finalImagePath);
            }
            finishImagesWithSuccess(finalImagesPath);
        } else {
            throw new IllegalStateException("Received image from picker that was not requested");
        }
    }
    ...
    private void finishImagesWithSuccess(ArrayList<String> imagePaths) {
